 ...
 */
function doubleTrouble(arrayOfNumbers) {
  // Your code here
}
 ...
 */
function stringyIndexes(arrayOfStrings) {
  // Your code here
}
 ...
 */
function onlyTheEvenSurvive(arrayOfNumbers) {
  // Your code here
}
 ...
 */
function onlyTheEvenIndexedSurvive(arrayOfNumbers) {
  // Your code here
}
 ...
 * odd.
 */
function everyoneIsOdd(arrayOfNumbers) {
  // Your code here
}
 ...
 * `needle` inside
 */
function findTheNeedle(arrayOfStrings) {
  // Your code here
}
 ...
 */
function findTheNeedleIndex(arrayOfStrings) {
  // Your code here
}
 ...
 */
function someoneToLove(arrayOfStrings) {
  // Your code here
}
...
const compare = (received, expected) => {
  if (deepEqual(received, expected)) {
    ...
    return `Received: ${JSON.stringify(received)} but expected ${JSON.stringify(expected)}`
  }
}
...
const tests = [
  ...
  },
  {
    ...
    test: () => {
      return compare(onlyTheEvenSurvive([42, 50, 100, 5, -43, 17, 44]), [42, 50, 100, 44])
    },
  },
  {
  ...
  },
  {
    ...
    test: () => {
      ...
      return (
        compare(bestMoviesOfTheYear(movies, 2014), ['The Grand Budapest Hotel', 'Birdman']) ||
        compare(bestMoviesOfTheYear(movies, 2017), ['Get Out', 'The Big Sick']) ||
  ...
  },
  {
    ...
    test: () => {
      return compare(everyoneIsOdd([9, 15, 27, 101, 33]), true) || compare(everyoneIsOdd([9, 23, 3, 4, 77]), false)
    },
  },
  {
    methodName: 'findTheNeedle',
    test: () => {
      return compare(findTheNeedle(['one', 'time', 'there was a needle at', 'the market']), 'there was a needle at')
    },
  },
  {
    methodName: 'findTheNeedleIndex',
    test: () => {
      return compare(findTheNeedleIndex(['one', 'time', 'there was a needle at', 'the market']), 2)
    },
  },
  {
  ...
  },
  {
    ...
    test: () => {
      ...
      const answer = Object.keys(objectOfHobbies).map(key => `${key} - ${objectOfHobbies[key].title}`)
...
console.clear()
tests.forEach(test => {
  const paddedTestName = test.methodName + ' '.repeat(longestTestName - test.methodName.length)
...
if (totalPassing === tests.length) {
  ...
  console.log(chalk.yellow('At least one method is not working ... keep going. You got this!'))
}
